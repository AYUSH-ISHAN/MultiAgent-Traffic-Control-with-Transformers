...
class UPDeT(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.q_basic = nn.Linear(args.emb, 10)  # intially it was 6
    ...
    def forward(self, inputs, raw_obs, hidden_state, task_enemy_num, task_ally_num):
        ...
        # print("output in updet : ", outputs.shape)
        h = outputs[:, -1:, :]
        # print("hidden in updet : ", h.shape)
        q_enemies_list = []
...
def get_adjacency_matrix(obs):
        ...
        for agent in range(10):
            for i in range(agent):  # already other half is marked below in index
                    # print(agent, i)
                    if(((obs[0][agent][2]-obs[0][i][2])**2 +(obs[0][agent][3]-obs[0][i][3])**2) < 1):
                        adj[agent][i] = 1.0
                        adj[i][agent]=1.0
        return adj
...
class Transformer(nn.Module):
    ...
    def forward(self, x, h, mask):
        ...
        # h = h.permute(1,0,2)  # permuting for dimension fit
        # print(tokens.shape)
        # print(h.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ally_num', default='10', type=int)
    parser.add_argument('--enemy_num', default='0', type=int)
