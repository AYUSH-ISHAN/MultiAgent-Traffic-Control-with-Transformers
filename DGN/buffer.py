...
class ReplayBuffer(object):
	...
	def getBatch(self, batch_size):
		...
		return self.obs[index], self.actions[index], self.rewards[index], self.next_obs[index],self.matrix[index],self.next_matrix[index],self.dones[index]#self.next_mask[index], self.dones[index]

	def add(self, obs, action, reward, next_obs, matrix, next_matrix, done): #next_mask, done):
